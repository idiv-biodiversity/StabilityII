...
mod2<-read.delim("/home/dylan/Dropbox/leipzigPhyTrt/StabilityII_data/Community_Level/Rst_WET_empd_fdis_sem_coefs_NEW.csv",sep=",",header=TRUE)
#mod11<-filter(mod1,response=="TS_lg2")
#mod11$Model<-"eMPD"
#mod11$ModelGroup<-"FD"
...
paths$response<-ifelse(paths$response=="Yn_PCAcwm4trts","F-S",paths$response)
paths$response<-ifelse(paths$response=="Yn_eMPD","PD",paths$response)
paths$response<-ifelse(paths$response=="Yn_FDis4","FD",paths$response)
paths$response<-ifelse(paths$response=="Plot_Asynchrony","&#951;",paths$response)
paths$response<-ifelse(paths$response=="lg2Rst12","Rst",paths$response)
paths$predictor<-ifelse(paths$predictor=="lg2SppN","SR",paths$predictor)
#paths$predictor<-ifelse(paths$predictor=="Plot_Asynchrony","&#951;",paths$predictor)
paths$predictor<-ifelse(paths$predictor=="Yn_PCAcwm4trts","F-S",paths$predictor)
paths$predictor<-ifelse(paths$predictor=="Yn_FDis4","FD",paths$predictor)
paths$predictor<-ifelse(paths$predictor=="Yn_eMPD","PD",paths$predictor)
...
paths$response<-ifelse(paths$response=="Ye_PCAcwm4trts","F-S",paths$response)
paths$response<-ifelse(paths$response=="Ye_eMPD","PD",paths$response)
paths$response<-ifelse(paths$response=="Ye_FDis4","FD",paths$response)
paths$response<-ifelse(paths$response=="lg2Rsl12","Rsl",paths$response)
#paths$response<-ifelse(paths$response=="TS_lg2","1/CV",paths$response)
paths$predictor<-ifelse(paths$predictor=="lg2SppN","SR",paths$predictor)
paths$predictor<-ifelse(paths$predictor=="Ye_PCAcwm4trts","F-S",paths$predictor)
paths$predictor<-ifelse(paths$predictor=="Ye_FDis4","FD",paths$predictor)
paths$predictor<-ifelse(paths$predictor=="Ye_eMPD","PD",paths$predictor)
